...
func TestIncrDecrArgErrors(t *testing.T) {
	...
	tc.mustFail("incr", "usage: incr <pk> [delta] [ttlms]")
	tc.mustFail("incr c 1 2 3", "usage: incr <pk> [delta] [ttlms]")
	tc.mustFail("incr c 1 0", "ttlms must be a positive integer")
	tc.mustFail("incr c 1 -5", "ttlms must be a positive integer")
...
func TestTopupTakeArgErrors(t *testing.T) {
	...
	tc.mustFail("topup c", "usage: topup <pk> <amount> <max> [ttlms]")
	tc.mustFail("topup c 1", "usage: topup <pk> <amount> <max> [ttlms]")
	tc.mustFail("topup c 1 2 3 4", "usage: topup <pk> <amount> <max> [ttlms]")
	tc.mustFail("topup c 1 2 x", "ttlms must be a positive integer")
	tc.mustFail("take c 1", "usage: take <pk> <amount> <left> [ttlms]")
